...
export default {
  ...
  methods: {
    // onChange (...args) { // args表示所有参数，三个点表示把所有参数收集到数组中
    onChange (event) {
      // 打印所有
      // console.log(args);打印结果[Event]   只有一个参数，如果是多个，会依次列出来，
      // Event就是事件对象
      // 所以此处直接打印event
      // console.log(event)  事件对象里面有一个target，里面有DOM元素-inputinput里面包含-files,数组，（因为可以选择多个文件，所以默认是一个数组）我们只要里面的一个
      // 获取到选中的图片
      ...
      console.log(11, file, fileUrl)
      this.src = fileUrl // 赋给src，再通过组件的:src传递给子组件
      // 展示预览图片弹出层
      this.isUpdatePhotoShow = true

      // file-input 如果选了同一个文件不会触发 change 事件
      // 解决办法就是每次使用完毕，把它的 value 清空
      // 选中文件后 把文本域的值清空，这样下次选择同一个文件时才会触发change事件
