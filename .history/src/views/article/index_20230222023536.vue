...
-->
        ...
        <!-- 给文章评论组件传递评论列表数据`:list="commentList"` -->
        <!-- 评论的回复：显示弹层，先监听reply-click事件，然后处理函数中显示弹出层 -->
        <CommentList
          ref="CommentIcon"
        ...
        <div class="article-bottom">
          ...
          >
            ...
            <!-- 弹层渲染出来以后就只是简单的切换显示和隐藏，里面的内容也不再重新渲染了 -->
            <!-- v-if="isReplyShow"  //每次弹层显示的时候重新渲染组件   可以解决评论的回复列表不会动态更新的问题 -->
            <!-- 但是这里我们尝试使用另外一种办法 -->
            <comment-reply
              :is-reply-show="isReplyShow" 
          ...
          <!-- /评论的回复 -->
          <!-- 评论图标 -->
          <!-- 添加了一个滚动效果，显示利用ref获取到dom节点，然后利用ref获取到组件，然后使用behavior的smooth呈现一种缓慢的滚动效果。 -->
          <van-icon
            name="comment-o"
